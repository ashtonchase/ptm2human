...
int decode_etb_stream(struct stream *etb_stream) {
	...
	int ret = 0;
	int nr_stream, pkt_idx, byte_idx, id, cur_id, pre_id, nr_new, i,
			trace_stop = 0;
	...
	if (1 == etb_stream->tracer.ptm.raw) {
		decode_stream(etb_stream);
	} else {
		...
		for (pkt_idx = 0; pkt_idx < etb_stream->buff_len; pkt_idx +=
				ETB_PACKET_SIZE) {
			if (trace_stop) {
				break;
			}
					...
					if (c & 1) {
						...
						if (cur_id >= nr_stream) {
							...
							stream = realloc(stream,
									sizeof(struct stream) * nr_stream);
							if (!stream) {
